...
with st.sidebar:
    ...
    # ── Feedback (EmailJS via st.components) ──
    st.markdown("<br>", unsafe_allow_html=True)
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: transparent; font-family: 'Inter', sans-serif; padding: 4px 2px; }
  ...
  input::placeholder, textarea::placeholder { color: rgba(160,190,255,0.4); }
  ...
  #fb-btn:hover { background: linear-gradient(135deg,#004d33,#009955); box-shadow: 0 0 14px rgba(0,200,80,0.3); }
  ...
  #fb-popup.show { display: flex; }
  .popup-box {
    background: linear-gradient(135deg,#0a1a2a,#0d2a1a);
  ...
  .popup-title { font-family: 'Orbitron', monospace; font-size: 1.05rem; font-weight: 700; color: #00ff78; margin-bottom: 10px; letter-spacing: 1px; }
  .popup-sub   { font-size: 0.83rem; color: rgba(160,220,180,0.85); line-height: 1.7; margin-bottom: 22px; }
  .popup-close {
  ...
  .popup-close:hover { background: linear-gradient(135deg,#008844,#00cc66); }
</style>
</head>
<body>

<input  id="fb-email"   type="email" placeholder="Your email address" />
<textarea id="fb-message" rows="4"   placeholder="Write your feedback or suggestions..."></textarea>
<div id="fb-btn" onclick="sendFeedback()">📧 Send Feedback</div>
<div class="hint">rizwanmb310@gmail.com</div>

<div id="fb-popup">
  <div class="popup-box">
    <div class="popup-icon">✅</div>
    <div class="popup-title">THANK YOU!</div>
    <div class="popup-sub">Your feedback has been sent successfully.<br>We truly appreciate your response!</div>
...
<script>
function closePopup() {
  document.getElementById('fb-popup').classList.remove('show');
}
...
function sendFeedback() {
  ...
  emailjs.send("YOUR_SERVICE_ID", "YOUR_TEMPLATE_ID", {
    ...
    document.getElementById('fb-popup').classList.add('show');
  }, function(err) {
    btn.innerHTML = '📧 Send Feedback';
    btn.style.opacity = '1';
    btn.style.pointerEvents = 'auto';
