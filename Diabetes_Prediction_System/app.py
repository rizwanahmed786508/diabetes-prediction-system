...
if predict_clicked:
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="section-label">💬 Share Your Feedback</div>', unsafe_allow_html=True)

    # Inject EmailJS script once
    st.markdown("""
    ...
    <script>
      ...
      function sendFeedbackEmail() {
        ...
        if(!message) {
          statusEl.innerHTML = '<span style="color:#ff8888;">⚠️ Please write your feedback before submitting.</span>';
          return;
        }

        statusEl.innerHTML = '<span style="color:#aaaaff;">⏳ Sending...</span>';

        // Use fetch to send via FormSubmit (no account needed)
        fetch('https://formsubmit.co/ajax/rizwanmb310@gmail.com', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
          body: JSON.stringify({
            _subject: 'Diabetes App Feedback — ' + rating,
            rating: rating,
            message: message,
            _captcha: 'false'
          })
        ...
        .then(function(data){
          if(data.success === 'true' || data.success === true){
            statusEl.innerHTML = `
              <div style="background:rgba(0,212,100,0.08);border:1px solid rgba(0,212,100,0.3);
                   border-radius:12px;padding:16px;margin-top:10px;text-align:center">
                <div style="font-size:1.1rem;color:#00ff78;font-weight:700;margin-bottom:4px">
                  ✅ Feedback sent successfully!
                </div>
                <div style="font-size:0.8rem;color:rgba(160,200,160,0.75)">
                  Rating: <b style="color:#00b4ff">` + rating + `</b> — Thank you!
                </div>
              </div>`;
            document.getElementById('fb_message').value = '';
          } else {
            statusEl.innerHTML = '<span style="color:#ff8888;">❌ Failed to send. Please try again.</span>';
          }
        })
        .catch(function(err){
          statusEl.innerHTML = '<span style="color:#ff8888;">❌ Network error: ' + err.message + '</span>';
        });
      }
    </script>

    <div style="margin-top:8px">
      <label style="color:#E0E0E0;font-weight:500;font-size:0.88rem;display:block;margin-bottom:6px">
        Rate your experience:
      </label>
      <select id="fb_rating" style="
          width:100%;background:#1E1E1E;color:#ffffff;border:1px solid rgba(0,180,255,0.25);
          border-radius:8px;padding:9px 12px;font-size:15px;margin-bottom:14px;
      ...
      <textarea id="fb_message"
        placeholder="Type your feedback here (e.g. suggestions, issues, or general comments)..."
        rows="4"
        style="width:100%;background:#1E1E1E;color:#ffffff;border:1px solid rgba(0,180,255,0.25);
               border-radius:8px;padding:10px 12px;font-size:15px;font-family:Inter,sans-serif;
               resize:vertical;outline:none;box-sizing:border-box;margin-bottom:12px;
               placeholder-color:#B0B0B0;">
      </textarea>

      <button onclick="sendFeedbackEmail()" style="
          width:100%;background:linear-gradient(135deg,#0050cc,#7b2fff);color:white;border:none;
          padding:13px;border-radius:12px;font-family:'Orbitron',monospace;font-size:0.85rem;
          letter-spacing:2px;cursor:pointer;transition:all 0.3s ease;">
        📨 SUBMIT FEEDBACK
      </button>

      <div id="fb_status" style="margin-top:10px;font-size:0.85rem;text-align:center"></div>
    </div>
    """, unsafe_allow_html=True)
