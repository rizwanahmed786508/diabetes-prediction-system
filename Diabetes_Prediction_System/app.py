    ...
    if st.button("📨 SUBMIT FEEDBACK", key="submit_feedback"):
        if not feedback_text.strip():
            ...
            try:
                ...
                SENDER_EMAIL = "rizwanmb310@gmail.com"
                SENDER_PASS  = st.secrets["EMAIL_PASS"]
                RECV_EMAIL   = "rizwanmb310@gmail.com"

                msg = MIMEMultipart("alternative")
                msg["Subject"] = f"Diabetes App Feedback — {feedback_rating}"
                ...
                <html><body style="font-family:Arial,sans-serif;background:#f4f4f4;padding:20px">
                  <div style="max-width:520px;margin:auto;background:white;border-radius:12px;
                              padding:28px;border-top:4px solid #00b4ff">
                    <h2 style="color:#0d1420;margin-top:0">New App Feedback</h2>
                    <p><b>Rating:</b> <span style="color:#0050cc">{feedback_rating}</span></p>
                    <p><b>Message:</b></p>
                    <div style="background:#f0f6ff;border-left:4px solid #00b4ff;
                                padding:12px 16px;border-radius:6px;color:#1a2a3a">
                      {feedback_text.strip().replace(chr(10), "<br>")}
                    </div>
                    <hr style="margin-top:24px;border:none;border-top:1px solid #e0e0e0">
                    <p style="font-size:12px;color:#888">Sent from Diabetes Prediction System</p>
                  </div>
                </body></html>
                """
                ...
                st.success("✅ Feedback sent successfully! Jazakallah for your response.")
                st.balloons()

            except KeyError:
                st.error("⚙️ EMAIL_PASS secret not set. Add it in Streamlit Cloud → Settings → Secrets.")
            except smtplib.SMTPAuthenticationError:
                st.error("❌ Gmail login failed. Make sure you are using a Gmail App Password.")
