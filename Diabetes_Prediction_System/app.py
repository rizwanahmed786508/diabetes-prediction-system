...
if predict_clicked:
    ...
    feedback_text = st.text_area(
        "We'd love to hear your thoughts about this tool:",
    ...
    if st.button("📨 Submit Feedback", key="submit_feedback"):
        if feedback_text.strip():
            mailto_body = (
                f"Rating: {feedback_rating}%0A%0A"
                f"Feedback:%0A{feedback_text.strip().replace(' ', '%20').replace(chr(10), '%0A')}"
            )
            mailto_link = f"mailto:rizwanmb310@gmail.com?subject=Diabetes%20Prediction%20App%20Feedback&body={mailto_body}"
            st.markdown(f"""
            <div style="background:rgba(0,212,100,0.08);border:1px solid rgba(0,212,100,0.3);
                ...
                <div style="font-size:1.1rem;color:#00ff78;font-weight:700;margin-bottom:6px">
                    ✅ Thank you for your feedback!
                </div>
                <div style="font-size:0.82rem;color:rgba(160,200,160,0.8);margin-bottom:12px">
                    Rating: <b style="color:#00b4ff">{feedback_rating}</b>
                </div>
                <a href="{mailto_link}" target="_blank"
                   style="display:inline-block;background:linear-gradient(135deg,#0050cc,#7b2fff);
                          color:white;text-decoration:none;padding:9px 22px;border-radius:10px;
                          font-size:0.82rem;letter-spacing:1px;font-family:'Orbitron',monospace">
                    📧 Send via Email
                </a>
                <div style="font-size:0.72rem;color:rgba(160,190,255,0.45);margin-top:8px">
                    Click above to send feedback to rizwanmb310@gmail.com
                </div>
            </div>
            """, unsafe_allow_html=True)
        else:
