...
<style>
    ...
    @media (max-width: 768px) {
        div[data-testid="stNumberInput"] input { font-size: 17px !important; }
    }
...
def generate_pdf(patient_data, prediction, diabetic_prob, nondiabetic_prob,
    ...
    C_DARK=colors.HexColor("#0d1420"); C_BLUE=colors.HexColor("#00b4ff")
    C_GREEN=colors.HexColor("#00c864"); C_RED=colors.HexColor("#ff4444")
    C_TEXT=colors.HexColor("#1a2a3a"); C_MUTED=colors.HexColor("#5a7090")
    C_BORDER=colors.HexColor("#d0e4f0"); C_LBLUE=colors.HexColor("#e8f4ff")
    def S(name, **kw):
    ...
    story.append(Paragraph(
        f"Generated: {now.strftime('%B %d, %Y')} &nbsp;|&nbsp; Time: {now.strftime('%H:%M:%S')} &nbsp;|&nbsp; Model: {selected_model}",
    ...
    if prediction == 1:
        ...
        bg_color=colors.HexColor("#fff0f0"); txt_color=C_RED; border_c=C_RED
    else:
        res_label="NON-DIABETIC"; res_conf=f"{nondiabetic_prob}%"
        res_sub=f"Diabetic Probability: {diabetic_prob}%"
        bg_color=colors.HexColor("#f0fff6"); txt_color=C_GREEN; border_c=C_GREEN
    result_data = [[
    ...
    fig.patch.set_facecolor("white"); ax.set_facecolor("#f7faff")
    ...
    for i, (label, val) in enumerate(zip(sorted_labels[::-1], sorted_imp[::-1])):
        ax.text(val+0.003, i, f"{val:.3f}", va="center", ha="left", fontsize=7.5, color="#334466", fontweight="600")
    ...
    ax.spines["left"].set_color("#ccddee"); ax.spines["bottom"].set_color("#ccddee")
    ax.xaxis.grid(True, color="#ddeeff", linewidth=0.6, linestyle="--"); ax.set_axisbelow(True)
...
with st.sidebar:
    ...
    <input id="fb-email" type="email" placeholder="Your email address"
        style="width:100%;box-sizing:border-box;background:#111827;border:1px solid rgba(0,180,255,0.25);
               border-radius:8px;padding:9px 12px;color:#e0e6ff;font-size:0.82rem;
               margin-bottom:8px;outline:none;font-family:'Inter',sans-serif;" />
    <textarea id="fb-message" placeholder="Write your feedback or suggestions..." rows="4"
        style="width:100%;box-sizing:border-box;background:#111827;border:1px solid rgba(0,180,255,0.25);
               border-radius:8px;padding:9px 12px;color:#e0e6ff;font-size:0.82rem;resize:vertical;
               outline:none;font-family:'Inter',sans-serif;margin-bottom:8px;"></textarea>
    <div id="fb-btn" onclick="sendFeedback()"
        style="background:linear-gradient(135deg,#003322,#006644);border:1px solid rgba(0,200,100,0.35);
               border-radius:10px;padding:10px 14px;text-align:center;font-size:0.8rem;font-weight:600;
               color:#00ee77;cursor:pointer;letter-spacing:0.5px;transition:all 0.3s;">
        📧 Send Feedback
    </div>
    <div style="font-size:0.68rem;color:rgba(120,160,200,0.45);text-align:center;margin-top:6px">
        rizwanmb310@gmail.com
    </div>

    <div id="fb-popup" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;
         z-index:99999;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);
         align-items:center;justify-content:center;">
        <div style="background:linear-gradient(135deg,#0a1a2a,#0d2a1a);
                    border:1px solid rgba(0,255,120,0.45);border-radius:20px;
                    padding:44px 40px;text-align:center;max-width:300px;margin:auto;
                    box-shadow:0 0 70px rgba(0,255,120,0.25);">
            <div style="font-size:3rem;margin-bottom:14px;">✅</div>
            <div style="font-family:'Orbitron',monospace;font-size:1.05rem;font-weight:700;
                        color:#00ff78;margin-bottom:10px;letter-spacing:1px;">THANK YOU!</div>
            <div style="font-size:0.83rem;color:rgba(160,220,180,0.8);line-height:1.7;margin-bottom:22px;">
                Your feedback has been sent successfully.<br>We truly appreciate your response!
            </div>
            <div onclick="document.getElementById('fb-popup').style.display='none'"
                 style="background:linear-gradient(135deg,#006633,#00aa55);border-radius:10px;
                        padding:9px 28px;color:white;cursor:pointer;font-size:0.82rem;
                        font-weight:600;display:inline-block;letter-spacing:0.5px;">
                Close
            </div>
        </div>
    </div>
    ...
    function sendFeedback() {
        ...
        if (!email || !message) {
            alert('Please fill in both your email and feedback before sending.');
            return;
        }
        var re = /^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/;
        ...
        emailjs.send("YOUR_SERVICE_ID", "YOUR_TEMPLATE_ID", {
            from_email: email,
            message:    message,
            to_email:   "rizwanmb310@gmail.com"
        }).then(function() {
            ...
            var p = document.getElementById('fb-popup');
...
if predict_clicked:
    ...
    if prediction == 1:
        ...
        <div class="result-diabetic">
            ...
            <br>
            <div style="font-family:Orbitron,monospace;font-size:2rem;color:#ff5555;font-weight:700">{diabetic_prob}%</div>
        ...
        <div class="result-safe">
            ...
            <br>
            <div style="font-family:Orbitron,monospace;font-size:2rem;color:#00ff78;font-weight:700">{nondiabetic_prob}%</div>
    ...
    <div style="margin-bottom:12px">
        ...
        <div style="background:rgba(255,255,255,0.06);border-radius:6px;height:10px;overflow:hidden">
            <div style="width:{int(diabetic_prob)}%;background:linear-gradient(90deg,#cc0000,#ff5555);height:100%;border-radius:6px"></div>
        </div>
    </div>
    <div>
        <div style="display:flex;justify-content:space-between;font-size:0.8rem;margin-bottom:4px">
        ...
        <div style="background:rgba(255,255,255,0.06);border-radius:6px;height:10px;overflow:hidden">
            <div style="width:{int(nondiabetic_prob)}%;background:linear-gradient(90deg,#009944,#00ff78);height:100%;border-radius:6px"></div>
        </div>
    </div>
    <br>
    ...
    for rank in range(len(sorted_imp)):
        if rank == 0:   bar_colors.append("#00b4ff")
        elif rank == 1: bar_colors.append("#7b2fff")
        elif rank == 2: bar_colors.append("#ff6b35")
        else:           bar_colors.append("#2a4a7a")

    fig, ax = plt.subplots(figsize=(8, 3.8))
    fig.patch.set_facecolor("#0d1420"); ax.set_facecolor("#0d1420")
    ax.barh(sorted_labels[::-1], sorted_imp[::-1], color=bar_colors[::-1], height=0.55, edgecolor="none")
    ax.barh(sorted_labels[::-1], sorted_imp[::-1], color=bar_colors[::-1], height=0.72, edgecolor="none", alpha=0.12)
    for bar, val in zip(ax.patches[:len(sorted_imp)], sorted_imp[::-1]):
        ax.text(val+0.004, bar.get_y()+bar.get_height()/2, f"{val:.3f}",
                va="center", ha="left", fontsize=8, color="#a0c8ff", fontweight="600")
    ...
    ax.spines["top"].set_visible(False); ax.spines["right"].set_visible(False)
    ax.spines["left"].set_color("#1e3050"); ax.spines["bottom"].set_color("#1e3050")
    ax.xaxis.grid(True, color="#1a2a40", linewidth=0.6, linestyle="--"); ax.set_axisbelow(True)
    patches = [
        mpatches.Patch(color="#00b4ff", label=f"1st: {sorted_labels[0].replace(chr(10),' ')}"),
        mpatches.Patch(color="#7b2fff", label=f"2nd: {sorted_labels[1].replace(chr(10),' ')}"),
        mpatches.Patch(color="#ff6b35", label=f"3rd: {sorted_labels[2].replace(chr(10),' ')}"),
    ]
    ax.legend(handles=patches, loc="lower right", fontsize=7.5, framealpha=0.15,
              labelcolor="#c0d8ff", edgecolor="#1e3050", facecolor="#0a1020")
    ...
    with st.spinner("📝 Preparing PDF report..."):
        pdf_bytes = generate_pdf(
            ...
            selected_model=selected_model, model_accuracy=accuracies[selected_model],
            importances=importances, sorted_labels=sorted_labels, sorted_imp=sorted_imp,
        )
